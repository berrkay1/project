...
function AracSarjSistemleri() {

    const {valuesAracSarj} = useContext(MarsisContext);
    ...
    const addItem = (item) => {
        ...
        let button = document.querySelectorAll('[data-id="' + item.id + '"]');
        if(button[0].value !== ""){
            const newsepet = [
                ...sepet, 
                {id:item.id,ürün:item.ürün,fiyat:item.fiyat, adet:button[0].value}
            ]
            setsepet(newsepet);
        }
    ...
    <div className="catogories">
            ...
            <table className="table table-hover table-striped">
                ...
                <tbody>
                    {valuesAracSarj.map((item)=>(
                        ...
                        <td>{item.fiyat}</td>
                        <td>
                            <input data-id={item.id} id='count' type="number" min="1"  name='count'  />
                        </td>
                        <td>
                            <button type="button" 
                             onClick={()=>addItem(item, item.id)} className="btn btn-danger">Ekle</button>
                        </td>
                    </tr>
                    ))
        ...
        <table className="table table-hover table-striped">
                                ...
                                <tbody>
                                    {sepet.map(sepet => (
                                        <tr>
                                        <td data-urun={sepet.id}>{sepet.id}</td>
                                        <td>{sepet.ürün}</td>
                                        <td>{sepet.fiyat}</td>
                                        <td>{sepet.adet}</td>
                                        <td>
...
export default AracSarjSistemleri